# Google Drive V3 Provider Specification

## Purpose

This specification defines the requirements for the **Google Drive V3 Provider**, which is a concrete implementation of the BaseCloudProvider interface for Google Drive API v3. This provider enables file synchronization between OPFS and Google Drive using API v3 endpoints. This specification explicitly implements the interface defined in the opfs-cloud-file baseline specification and is a separate capability from the Google Drive V2 provider due to API version differences.

## Scope

This specification covers:
- Google Drive API v3 provider implementation
- Authentication and authorization requirements
- API endpoints and request formats specific to v3
- File metadata management
- File download and upload operations
- Change detection mechanisms using Google Drive v3

This specification does NOT cover:
- Core opfs-cloud-file library behavior (covered by opfs-cloud-file baseline spec)
- Google Drive API v2 (separate provider specification)
...
classDiagram
    ...
    BaseCloudProvider <|-- GoogleDriveV3Provider : implements
```
*Caption: Google Drive V3 Provider class diagram showing inheritance from BaseCloudProvider*

## Requirements

### Requirement: Provider Instantiation

The Google Drive V3 Provider SHALL be instantiated with a configuration object containing required properties.
...
**Implementation**: `providers/google-drive-v3/GoogleDriveV3Provider.js:5-13`
**Verification**: Unit tests for constructor validation

```mermaid
sequenceDiagram
    participant Consumer
    participant GoogleDriveV3Provider
    
    Consumer->>GoogleDriveV3Provider: new GoogleDriveV3Provider(config)
    GoogleDriveV3Provider->>GoogleDriveV3Provider: Validate fileId and accessToken
    alt Valid config
        GoogleDriveV3Provider->>GoogleDriveV3Provider: Store config
        GoogleDriveV3Provider->>GoogleDriveV3Provider: Initialize state
        GoogleDriveV3Provider-->>Consumer: Provider instance
    else Invalid config
        GoogleDriveV3Provider-->>Consumer: Throw Error
    end
```
*Caption: Google Drive V3 Provider instantiation sequence with validation*

#### Scenario: Successful instantiation with required config
- **WHEN** `new GoogleDriveV3Provider({ fileId: 'abc123', accessToken: 'xyz789' })` is called
- **THEN** system creates and returns a GoogleDriveV3Provider instance

#### Scenario: Missing fileId throws error
- **WHEN** `new GoogleDriveV3Provider({ accessToken: 'xyz789' })` is called
- **THEN** system throws an error with message 'fileId and accessToken required for Google Drive v3'

#### Scenario: Missing accessToken throws error
- **WHEN** `new GoogleDriveV3Provider({ fileId: 'abc123' })` is called
- **THEN** system throws an error with message 'fileId and accessToken required for Google Drive v3'

#### Scenario: Custom pollIntervalMs used
- **WHEN** `new GoogleDriveV3Provider({ fileId: 'abc123', accessToken: 'xyz789', pollIntervalMs: 5000 })` is called
...
The Google Drive V3 Provider SHALL support polling for remote file changes.

The `supportsPolling()` method SHALL return `true`.

**Implementation**: `providers/google-drive-v3/GoogleDriveV3Provider.js:15`
...
### Requirement: Metadata Fetching with Selective Fields

The Google Drive V3 Provider SHALL provide a method to fetch file metadata from Google Drive API v3 with selective field retrieval.

The `getFileMetadata()` method SHALL:
- Construct the API URL as `https://www.googleapis.com/drive/v3/files/{fileId}?fields=id,name,md5Checksum,modifiedTime,mimeType`
...
**Implementation**: `providers/google-drive-v3/GoogleDriveV3Provider.js:17-23`
...
sequenceDiagram
    ...
    Provider->>GoogleDriveAPI: GET /drive/v3/files/{fileId}?fields=id,name,md5Checksum,modifiedTime,mimeType
...
```
*Caption: Metadata fetching sequence with selective field retrieval and error handling*

#### Scenario: Metadata fetched with selective fields
- **WHEN** `provider.getFileMetadata()` is called with valid fileId and accessToken
- **THEN** system makes GET request to Google Drive API v3 with fields parameter and returns metadata object

#### Scenario: Metadata fetch with correct URL and fields
- **WHEN** `provider.getFileMetadata()` is called with fileId 'test-file-id'
- **THEN** system makes request to `https://www.googleapis.com/drive/v3/files/test-file-id?fields=id,name,md5Checksum,modifiedTime,mimeType`
...
The Google Drive V3 Provider SHALL retrieve the file name from Google Drive metadata.

The `getFileName()` method SHALL:
- Call `getFileMetadata()` to fetch the metadata
- Return the `name` property from the metadata if it exists
- Return `null` if the metadata or name is missing

**Note**: This is different from V2 which uses the `title` field.

**Implementation**: `providers/google-drive-v3/GoogleDriveV3Provider.js:25-28`
**Verification**: Unit tests for file name retrieval

```mermaid
flowchart TD
    A[getFileName] --> B[getFileMetadata]
    B --> C{Metadata has name?}
    C -->|Yes| D[Return metadata.name]
    C -->|No| E[Return null]
```
*Caption: File name retrieval flowchart with metadata fetching - uses 'name' field for V3*

#### Scenario: Return file name from metadata
- **WHEN** `provider.getFileName()` is called and metadata contains `{ name: 'test.txt', ... }`
- **THEN** system returns 'test.txt'

#### Scenario: Return null when metadata has no name
- **WHEN** `provider.getFileName()` is called and metadata contains `{ md5Checksum: 'abc' }` without name
...
The Google Drive V3 Provider SHALL download file content from Google Drive.
...
- Construct the download URL as `https://www.googleapis.com/drive/v3/files/{fileId}?alt=media`
- Send a GET request with the Authorization header: `Bearer {accessToken}`
- If the response is not OK, throw an error with the status code
- Return the response as an ArrayBuffer

**Implementation**: `providers/google-drive-v3/GoogleDriveV3Provider.js:41-55`
...
sequenceDiagram
    ...
    alt Is Google Apps file
        ...
        Provider->>GoogleDriveAPI: GET /drive/v3/files/{fileId}?alt=media
        Note over Provider,GoogleDriveAPI: Headers: Authorization: Bearer {accessToken}
...
- **THEN** system makes GET request to Google Drive API v3 with alt=media and returns ArrayBuffer
...
- **THEN** system makes request to `https://www.googleapis.com/drive/v3/files/test-file-id?alt=media`
...
### Requirement: File Upload with PATCH

The Google Drive V3 Provider SHALL upload file content to Google Drive using HTTP PATCH method.

The `upload(data)` method SHALL:
- Construct the upload URL as `https://www.googleapis.com/upload/drive/v3/files/{fileId}?uploadType=media`
...
**Note**: This is a key difference from V2 which uses PUT method.

**Implementation**: `providers/google-drive-v3/GoogleDriveV3Provider.js:41-56`
...
sequenceDiagram
    ...
    Provider->>GoogleDriveAPI: PATCH /upload/drive/v3/files/{fileId}?uploadType=media
    Note over Provider,GoogleDriveAPI: Headers: Authorization: Bearer {accessToken}
...
```
*Caption: File upload sequence with PATCH method and metadata storage - key difference from V2*

#### Scenario: Upload file content successfully with PATCH
- **WHEN** `provider.upload(data)` is called with valid data, fileId, and accessToken
- **THEN** system makes PATCH request to Google Drive API v3 upload endpoint and stores response metadata

#### Scenario: Upload with correct URL
- **WHEN** `provider.upload(data)` is called with fileId 'test-file-id'
- **THEN** system makes request to `https://www.googleapis.com/upload/drive/v3/files/test-file-id?uploadType=media`

#### Scenario: Upload with correct method (PATCH)
- **WHEN** `provider.upload(data)` is called
- **THEN** system uses HTTP PATCH method for the request
...
The Google Drive V3 Provider SHALL detect remote file changes using MD5 checksum comparison.
...
**Implementation**: `providers/google-drive-v3/GoogleDriveV3Provider.js:58-68`
...
The Google Drive V3 Provider SHALL provide access to the last known remote file checksum.
...
**Implementation**: `providers/google-drive-v3/GoogleDriveV3Provider.js:70-72`
...
The Google Drive V3 Provider SHALL compute checksums for local data using the MD5 algorithm.
...
**Implementation**: `providers/google-drive-v3/GoogleDriveV3Provider.js:74-80`
...
The Google Drive V3 Provider SHALL provide a method for resource cleanup.
...
The Google Drive V3 Provider SHALL fully implement the BaseCloudProvider interface.
...
- `getFileName()` - Returns Promise<string|null> with file name
...
**Implementation**: `providers/google-drive-v3/GoogleDriveV3Provider.js:4-81`
...
classDiagram
    ...
    BaseCloudProvider <|-- GoogleDriveV3Provider : implements all methods
```
...
The Google Drive V3 Provider SHALL be registered as a valid provider type in the core library.
...
The core library SHALL create an instance of GoogleDriveV3Provider when type `'google-drive-v3'` is specified.

**Implementation**: `providers/google-drive-v3/index.js:1`, `src/OpfsCloudFile.js:16-17`
...
sequenceDiagram
    ...
    Consumer->>OpfsCloudFile:   type: 'google-drive-v3',
    Consumer->>OpfsCloudFile:   provider: { config: { fileId: '...', accessToken: '...' } }
    Consumer->>OpfsCloudFile: })
    OpfsCloudFile->>GoogleDriveV3Provider: new GoogleDriveV3Provider(config.provider.config)
    GoogleDriveV3Provider-->>OpfsCloudFile: Provider instance
    OpfsCloudFile-->>Consumer: OpfsCloudFile instance
```
*Caption: Provider type registration sequence for google-drive-v3 type*

#### Scenario: Provider created via type string
- **WHEN** `new OpfsCloudFile({ type: 'google-drive-v3', provider: { config: { fileId: 'abc', accessToken: 'xyz' } } })` is called
- **THEN** system creates GoogleDriveV3Provider instance and initializes OpfsCloudFile
...
|-------------|----------------|--------------|
| Provider Instantiation | `providers/google-drive-v3/GoogleDriveV3Provider.js:5-13` | Unit tests for constructor validation |
| Polling Support | `providers/google-drive-v3/GoogleDriveV3Provider.js:15` | Unit tests for polling support |
| Metadata Fetching | `providers/google-drive-v3/GoogleDriveV3Provider.js:17-23` | Unit tests for metadata fetching |
| File Name Retrieval | `providers/google-drive-v3/GoogleDriveV3Provider.js:25-28` | Unit tests for file name retrieval |
| File Download | `providers/google-drive-v3/GoogleDriveV3Provider.js:41-55` | Unit tests for enhanced error messages |
| File Upload | `providers/google-drive-v3/GoogleDriveV3Provider.js:41-56` | Unit tests for file upload |
| Change Polling | `providers/google-drive-v3/GoogleDriveV3Provider.js:58-68` | Unit tests for change polling |
| Remote File Checksum | `providers/google-drive-v3/GoogleDriveV3Provider.js:70-72` | Unit tests for remote checksum |
| Local Data Checksum | `providers/google-drive-v3/GoogleDriveV3Provider.js:74-80` | Unit tests for local checksum |
| Resource Cleanup | `providers/BaseCloudProvider.js:11` (inherited) | Interface compliance check |
| Interface Compliance | `providers/google-drive-v3/GoogleDriveV3Provider.js:4-81` | Provider interface validation |
| Provider Type Registration | `providers/google-drive-v3/index.js:1`, `src/OpfsCloudFile.js:16-17` | Integration tests |
