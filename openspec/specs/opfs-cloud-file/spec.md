...
- Implement retry logic with configurable retry configuration for transient errors

**Implementation**: `src/OpfsCloudFile.js:49-81` (upload flow), `src/OpfsCloudFile.js:127-140` (retry logic)
...
- Clean up all tracked OPFS access handles to prevent resource leaks

**Implementation**: `src/OpfsCloudFile.js:181-201`
...
**Implementation**: `utils/opfs.js:1-90`
...
**Implementation**: `utils/md5.js:1-13`
...
**Implementation**: `src/OpfsCloudFile.js:26-30` (constructor options), `src/OpfsCloudFile.js:127-140` (retry mechanism)
...
- When both local and remote checksums differ from last known state:
  ...
  - If timestamps are unavailable: Fall back to last-write-wins resolution (local wins)
...
**Implementation**: `src/OpfsCloudFile.js:86-110`
...
#### Scenario: Fallback when timestamps unavailable
- **WHEN** both local and remote files changed and either timestamp is unavailable
- **THEN** system falls back to last-write-wins resolution and uploads local changes
...
- On `stop()`: Close all handles in the Set and emit a warning for any that fail to close, as an `opfs-cloud-error` event with detail `{ warning: true, failures }`
- Provide a `cleanup()` method for explicit resource cleanup

**Implementation**: `utils/opfs.js:1-90`, `src/OpfsCloudFile.js:205-219`
...
- **THEN** system emits an opfs-cloud-error event with detail `{ warning: true, failures }` describing the failed cleanups
...
**Implementation**: `src/worker.ts:1-31`
...
| Local Change Handling | `src/OpfsCloudFile.js:49-81, 127-140` | Unit tests for local change upload flow with retry |
| Manual Synchronization | `src/OpfsCloudFile.js:84-96` | Unit tests for manual sync behavior |
| Start and Stop Control | `src/OpfsCloudFile.js:181-201` | Unit tests for start/stop behavior with resource tracking |
| File Download and Replace | `src/OpfsCloudFile.js:119-125` | Unit tests for download and replace behavior |
| OPFS File Operations | `utils/opfs.js:1-90` | Unit tests for OPFS file operations |
| Checksum Computation | `utils/md5.js:1-13` | Unit tests with known MD5 values |
| Error Recovery Configuration | `src/OpfsCloudFile.js:26-30, 127-140` | Unit tests for retry behavior |
| Conflict Detection and Resolution | `src/OpfsCloudFile.js:86-110` | Unit tests for conflict scenarios |
| Resource Tracking and Cleanup | `utils/opfs.js:1-90`, `src/OpfsCloudFile.js:205-219` | Unit tests for resource management |
| Worker Event Forwarding | `src/worker.ts:1-31` | Integration tests for worker events |
